...
export function rules(latest: Latest) {
  ...
  return {
    varied: `at least ${pct(v.unique_tpl)} different messages, at least ${pct(v.repeat_share)} of messages from senders who post more than once, no sender above ${pct(v.top_share)} of the messages, and at least ${v.eff_senders} effective senders`,
    repetitive: `under ${pct(r.unique_tpl)} different messages, one sender writing ${pct(r.top_share)} or more of the messages, or under ${pct(r.repeat_share)} of messages from senders who post more than once`,
